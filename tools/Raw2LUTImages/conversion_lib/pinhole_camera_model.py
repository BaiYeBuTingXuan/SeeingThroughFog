...
class PinholeCameraModel:
    ...
    def fromJsonDict(self, loadeddict):
        self.K = mkmat(3, 3, loadeddict['K'])
        if loadeddict.get('D', False):
            self.D = mkmat(len(loadeddict['D']), 1, loadeddict['D'])
        else:
            self.D = None
        self.R = mkmat(3, 3, loadeddict['R'])
        self.P = mkmat(3, 4, loadeddict['P'])
        self.full_K = mkmat(3, 3, loadeddict['K'])
        self.full_P = mkmat(3, 4, loadeddict['P'])
        self.width = loadeddict['width']
        self.height = loadeddict['height']
        self.binning_x = max(1, loadeddict['binning_x'])
        self.binning_y = max(1, loadeddict['binning_y'])
        self.resolution = (loadeddict['width'], loadeddict['height'])

        self.raw_roi = copy.copy(loadeddict['roi'])
        # ROI all zeros is considered the same as full resolution
        if (self.raw_roi['x_offset'] == 0 and self.raw_roi['y_offset'] == 0 and
            self.raw_roi['width'] == 0 and self.raw_roi['height'] == 0):
            self.raw_roi['width'] = self.width
            self.raw_roi['height'] = self.height
        self.tf_frame = loadeddict['header']['frame_id']
        self.stamp = loadeddict['header']['stamp']
    ...
    def rectifyImage(self, raw, rectified):
        ...
        cv2.remap(raw, self.mapx, self.mapy, cv2.INTER_AREA, rectified)
